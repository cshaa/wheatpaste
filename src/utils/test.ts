...
export interface Fn<ReturnValue, Args extends any[]>
  extends Mock<(...args: Args) => ReturnValue> {
  assertCalledOnce(args?: Args, ret?: ReturnValue): void;
  assertNotCalled(): void;
  assertCalledTimes(times: number): void;
}

export const fn = <R, A extends any[]>(f?: (...args: A) => R): Fn<R, A> => {
  return Object.assign(mock(f!), {
    assertCalledOnce(this: Fn<R, A>, args: A, ret?: R) {
      ...
      if (arguments.length > 1) {
        try {
          expect(this.mock.results[0].type).toBe("return");
          expect(this.mock.results[0].value).toStrictEqual(ret);
        } catch (cause) {
          throw new Error(
